...
public class SwerveModuleSim implements SwerveModuleIO {
	final private CANSparkMax drive;
	final private CANSparkMax steer;

	final private PIDController driveController;
	final private PIDController steerController;
	...
	public void periodic() {
		...
		double steerVoltage =
		MoreMath.minMax(
				steerController.calculate(encoder.getRot().getRadians(),targetState.angle.getRadians()),
				-3 * Math.PI, 3 * Math.PI
				);
